...
local function GetExploreTalking(mapId, heroTab)
    ...
    if _randomType <= 20 then
        if #_allTalkTab >= 1 then
            local index = math.random(1, #_allTalkTab);
            print(index,"--------");
            return _allTalkTab[index], true
        end
    end
    print(sprinttb(_talkTab));


    if not _talkTab or #_talkTab == 0 then
        return nil, false
    end
    local index = math.random(1, #_talkTab);
    print(index,"--------");
    return _talkTab[index], false
end

local guildBattleRewards = nil;
local function GetGuildBattleRewards(rank)
    if guildBattleRewards == nil then
        guildBattleRewards = LoadDatabaseWithKey("guildBattle_rewards", "rank");
    end
    return guildBattleRewards[rank];
end

local rank_boss = nil


local function GetUnionBossRank( order)
    

    if not rank_boss then
        rank_boss = rank_boss or {}
        DATABASE.ForEach("rank_boss", function(row)

            local reward = {};


            for i=1,3 do
                if row["reward_id"..i] ~=0 then
                    table.insert(reward,{ id = row["reward_id"..i],type = row["reward_type"..i],value = row["reward_value"..i] });
                end
            end
            rank_boss[row.Order] = rank_boss[row.Order] or {}


            row.reward = reward

            table.insert(rank_boss[row.Order],row);
        end)
    end

    return order and rank_boss[order] or {}; 
end
...
return {
    ...
    GetGuildBattleRewards = GetGuildBattleRewards,

    GetUnionBossRank = GetUnionBossRank,
}
