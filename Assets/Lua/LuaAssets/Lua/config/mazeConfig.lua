...
local maze_type_npc = nil
local function buildBaseInfo()
    if BaseConfig and maze_type_npc then
        return ;
    end
    BaseConfig = {}
    maze_type_npc = {}
    DATABASE.ForEach("team_activity_npc", function(row)
		...
		maze_type_npc[row.type] = maze_type_npc[row.type] or {};

		table.insert(maze_type_npc[row.type],row);
	end)
end

function mazeConfig.GetTypeInfo( type )
	buildBaseInfo();

	return maze_type_npc[type];
end
