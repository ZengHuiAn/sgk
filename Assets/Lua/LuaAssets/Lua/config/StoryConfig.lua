...
local StoryChoose_Conf = nil
local StoryChoose_ConfByQuest = nil
local function GetStoryChooseConf(gid,questid)
	if StoryChoose_Conf == nil then
		StoryChoose_Conf = {}
		StoryChoose_ConfByQuest = {}
		DATABASE.ForEach("story_choices", function(row)
			if StoryChoose_Conf[row.gid] == nil then
				StoryChoose_Conf[row.gid] = {}
			end
			if StoryChoose_ConfByQuest[row.depend_quest] == nil then
				StoryChoose_ConfByQuest[row.depend_quest] = {}
			end
			StoryChoose_Conf[row.gid][#StoryChoose_Conf[row.gid]+1] = row
			StoryChoose_ConfByQuest[row.depend_quest][#StoryChoose_ConfByQuest[row.depend_quest]+1] = row
		end)
	end
	if gid then
		return StoryChoose_Conf[gid]
	end
	if questid then
		return StoryChoose_ConfByQuest[questid]
	end
end
...
local function ShowStory(_id,_Fun,_state,_skipFlag,_closeFunc,isCorouine)
	if isCorouine then
		_Fun = _Fun
	else
	...
	-- [[
	if GetStoryConf(_id) == nil then
		showDlgMsg("剧情id ".. _id .."数据库中不存在", function()
			if _Fun then _Fun() end
		end)
		ERROR_LOG("剧情id ".. _id .."数据库中不存在", _Fun, _closeFunc, debug.traceback())
		return;
	end
	--]]

	local data = {id = _id,Function = _Fun,state = _state,skipFlag = _skipFlag,onClose = _closeFunc};
...
return {
	...
	GetStoryChooseConf = GetStoryChooseConf,
