...
function View:Start(data)
    ...
    module.RedDotModule.PlayRedAnim(self.view.friend.tip)
    self:upFriendChatRedDot()
...
function View:upFriendChatRedDot()
    self.view.friend.tip.gameObject:SetActive(module.RedDotModule.GetStatus(module.RedDotModule.Type.FriendChat.Chat))
end
...
function View:ChatPanelRef(data,ChatView,idx)
	...
	--print(desc.."->"..#self:string_segmentation(desc))
 	...
 	-- 	else
		...
		--  				DialogStack.PushPref("Tips/shareQuestTip",{quest,_pid},UnityEngine.GameObject.FindWithTag("UITopRoot").gameObject)
		-- 	 		else
		-- 	 			showDlgError(nil,"等级不足")
		-- 	 		end
		-- 	 	else
...
function View:onEvent(event,data)
	--ERROR_LOG("data===>>111111",sprinttb(data))
	...
	elseif event == "GUIDE_SHOW_MASK" then --zoe 引导调用showmask时关闭聊天窗口
		self.view[UnityEngine.Animator].enabled = false
        self.view.transform:DOLocalMove(Vector3(0, -1000,0), 0.3):OnComplete(function ( ... )
            CS.UnityEngine.GameObject.Destroy(self.view.gameObject)
        end)
	elseif event == "SystemMessageListRedDotChange" or event == "PrivateChatData_CHANGE" or event == "Mail_Delete_Succeed" then
		self:upFriendChatRedDot()
	end
end
