...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
...
#if UNITY_EDITOR
        public string _current_get_object_name = "";
        public string StackTraceToString() {
            ...
            for (int i = 1; i < frames.Length; i++) /* Ignore current StackTraceToString method...*/
            {
                ...
                sb.AppendLine(string.Format("   {0}:{1} {2}",
                    ...
                    method.Name, _current_get_object_name));
            }
            return "c# stack:\n" + sb.ToString();
        }
#endif
        ...
        public object CreateDelegateBridge(RealStatePtr L, Type delegateType, int idx)
        {
            ...
            string tr = "";
            LuaAPI.xlua_getglobal(L, "debug"); // 1
            LuaAPI.lua_pushstring(L, "traceback"); //2
            LuaAPI.xlua_pgettable(L, -2); //2
            if (LuaAPI.lua_isfunction(L, -1))
            {
                LuaAPI.lua_pcall(L, 0, 1, 0);
                tr = LuaAPI.lua_tostring(L, -1);

                if (tr == "stack traceback:") {
            ...
            try
            {
                ...
                {
#if false && UNITY_EDITOR
                    bridge = new DelegateBridge(reference, luaEnv, tr);
#else
        ...
        public string GetDelegateBridgeTraceback()
        {
            string str = "";
            foreach (var kv in delegate_bridges)
            {
                if (kv.Value.IsAlive)
                {
                    DelegateBridge b = kv.Value.Target as DelegateBridge;
                    if (b != null) {
                        str += b.firstValue.ToString();
                        str += "\t";
                        str += b.tr;
                        str += "\n";
                    }
